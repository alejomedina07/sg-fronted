...
export const SgButton = ( props: SgButtonProps ) => {
  const { confirmationMessage, confirmationTitle, onClickAction, onConfirm, onConfirmClose, sending, sendingText = 'Enviando...', label,
    color, variant, classes, type
  } = props;
  const { openConfirmationDialog } = useDialogs();

  const _onClickAction = () => {
    if(onConfirm) {
      openConfirmationDialog(confirmationMessage || 'Confirm', onConfirm, onConfirmClose, confirmationTitle || 'Confirm')
